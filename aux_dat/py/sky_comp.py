...
dat = np.loadtxt(os.environ['DESIMODEL'] + '/spectra/spec-sky.dat')
pl.plot(dat[:,0], dat[:,1], label=r'Sky-spec, $\lambda<10^4\AA$', c='k')

ss  = np.loadtxt(os.environ['AUX'] + '/SullivanSimcoe.txt')
...
if save:
  header = '#  Wave[A]  Flux [1.e-17 erg / (Angstrom arcsec2 cm2 s)]\n#  Extended with Sullivan and Simcoe (https://arxiv.org/abs/1207.0817)'
  pl.savetxt(os.environ['AUX'] + '/spec-sky-nearir.dat', output, header=header)
...
pl.legend()
pl.savefig(os.environ['AUX'] + '/plots/sky_comp.pdf')
